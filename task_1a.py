...
from utils import create_sample_pdf_1a # Correctly import the function
...
def classify_headings(scored_headings):
    if not scored_headings: return "Untitled Document", []
    title_info = scored_headings.pop(0)
    title = title_info['text']
    if not scored_headings: return title, []
    size_groups = defaultdict(list)
    for h in scored_headings: size_groups[h['size']].append(h)
    sorted_sizes = sorted(size_groups.keys(), reverse=True)
    outline, level_map = [], {0: "H1", 1: "H2", 2: "H3"}
...
def extract_structure(pdf_path: str) -> dict:
    ...
    except Exception as e: return {"error": f"Could not open PDF {pdf_path}: {e}"}
    ...
    for page_num, page in enumerate(doc):
        ...
        for block in blocks:
            if block['type'] == 0 and block.get("lines") and len(block.get("lines")) == 1:
                ...
                if not full_block_text or len(full_block_text.split()) > 15: continue
                first_span = line["spans"][0]
                font_size, is_bold = round(first_span["size"]), "bold" in first_span["font"].lower()
                score = 1.0
                if font_size > body_text_size: score *= (font_size / body_text_size)
                if is_bold: score *= 1.2
                if re.match(r'^((\d+\.)*\d+|[A-Z]\.)\s', full_block_text): score *= 1.5
                if full_block_text.endswith('.'): score *= 0.8
                if score > 1.25:
    ...
    if not title and doc.metadata.get('title'): title = doc.metadata['title']
...
if __name__ == '__main__':
    # Create the sample PDF for local testing
    create_sample_pdf_1a()
    
    pdf_to_process = "sample_document.pdf"
